...
export default {
    ...
    const toggleRepInputs = (activeField) => {
      if (activeField === 'number') {
        ...
        if (!editedEventRepNumber.value) editedEventRepDate.value = '' // Clear date input if number input is cleared
        ...
        if (!editedEventRepDate.value) editedEventRepNumber.value = '' // Clear number input if date input is cleared
      }
    }
